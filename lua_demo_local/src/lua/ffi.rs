...
impl Lua {
	...
	pub fn load_string(&self, code: &str) {
		let c_code = match CString::new(cstr!(code)) {
			Ok(s)  => s,
			Err(e) => return,
		};
		unsafe {
			let ret = luaL_loadstring(self.L, c_code.as_ptr() as *const i8 );
			check_ret(ret);
		}
	}
...
// }
	...
	pub fn register(&self, name: &str, function: lua_CFunction) {
		let c_name = match CString::new(cstr!(name)) {
			Ok(s)  => s,
			Err(e) => return,
		};
		unsafe {
			lua_pushcclosure(self.L, function, 0);
			lua_setglobal(self.L, c_name.as_ptr() as *const i8 )
		}
	}
	...
	pub fn to_string(&self, index: i32) -> String {
		...
		unsafe { rstr!(lua_tolstring(self.L, index, ptr::null_mut()) as *const u8) }
	}
